...
const { UserModel, ExerciseModel } = require('../models/exercise-tracker');
...
router.post('/exercise/add', async (req, res) => {
  try {
    ...
    let exercise = await ExerciseModel.create(
      properDate
        ? { userId, description, duration, date: properDate }
        : { userId, description, duration }
    );
    ...
    if (exercise) {
      ...
      let userWithExercise = {
        ...
        description,
        duration,
        date: new Date(date).toString().substr(0, 15),
      };
